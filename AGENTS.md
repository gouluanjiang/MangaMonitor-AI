...
- On 2026-09-09 the user authorized resource-aware local/cloud development to shorten feedback while preserving desktop responsiveness. Use local edits, formatting and targeted tests for quick feedback; keep full workspace/cross-platform validation, clean/release builds, installers and long soak runs in GitHub Actions. Batch relevant lightweight checks before pushing; do not repeat successful checks without new changes or unresolved concerns.
