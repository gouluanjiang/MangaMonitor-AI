...
This is a development shell, with native UI persistence implemented. Both real source logins and remembered-session restart have been accepted by the user; remaining live cover and website favorite-write checks are recorded in the [development handoff](../../docs/DEVELOPMENT_HANDOFF.md). A reader, ZIP packaging/verification, task authority, import, safe native task shutdown/recovery and cloud publication remain unfinished in this interface. Resource preferences are not yet connected to a real scheduler.
