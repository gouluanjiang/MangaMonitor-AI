...
pub(crate) fn live_execution_allowed() -> Result<(), StoreError> {
...
pub(crate) async fn download_inventory_read<R: Runtime>(
    ...
    let downloads = Arc::clone(downloads.inner());
    let store = open_store(Arc::clone(store.inner())).await?;
    // The store waits for its lock before reading; do not retry the entire
    // inventory operation or multiply the persistent-contention time budget.
