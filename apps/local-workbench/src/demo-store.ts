...
const readyWorkIds = new Set(works.filter((work) => work.status === 'ready').map((work) => work.id));
const demoError = '模拟连接中断，进度已保留。';
const taskStages = new Set<TaskStage>([
  'queued', 'downloading', 'verifying', 'packing', 'importing', 'sync_pending', 'completed', 'error',
]);
const localStages = new Set<TaskStage>(['queued', 'downloading', 'verifying', 'packing', 'importing']);
const fullProgressStages = new Set<TaskStage>(['verifying', 'packing', 'importing', 'sync_pending', 'completed']);

export function initialDemoState(): DemoState {
  return {
    version: 1,
    tasks: [
      { id: 'demo-sea', workId: 'sea', stage: 'downloading', progress: 35, paused: false, error: null },
      { id: 'demo-moon', workId: 'moon', stage: 'sync_pending', progress: 100, paused: false, error: null },
      { id: 'demo-train', workId: 'train', stage: 'error', progress: 27, paused: false, error: demoError },
    ],
    online: true,
    paused: false,
    closed: false,
...
export function enqueueWorks(state: DemoState, selectedWorks: Work[]): DemoState {
  ...
  for (const work of selectedWorks) {
    ...
    if (work.status !== 'ready' || !readyWorkIds.has(work.id) || existing.has(work.id)) continue;
    existing.add(work.id);
    additions.push({
      id: `demo-${work.id}`, workId: work.id, stage: 'queued', progress: 0, paused: false, error: null,
    });
  }
  return additions.length === 0 ? state : { ...state, tasks: [...state.tasks, ...additions] };
}
...
function advanceLocalTask(task: DemoTask): DemoTask {
  switch (task.stage) {
    ...
    case 'downloading': {
      ...
      return { ...task, progress, stage: progress === 100 ? 'verifying' : 'downloading' };
    }
    case 'verifying':
...
export function tickDemo(state: DemoState): DemoState {
  ...
  const localIndex = state.tasks.findIndex((task) => !task.paused && localStages.has(task.stage));
  const syncIndex = state.online
    ? state.tasks.findIndex((task) => !task.paused && task.stage === 'sync_pending')
    : -1;
...
export function toggleTaskPause(state: DemoState, id: string): DemoState {
  ...
  if (!task || task.stage === 'completed' || task.stage === 'error') return state;
  return {
    ...state,
    tasks: state.tasks.map<DemoTask>((item) => item.id === id ? { ...item, paused: !item.paused } : item),
  };
}
...
export function retryDemoTask(state: DemoState, id: string): DemoState {
  ...
  return {
    ...state,
    tasks: state.tasks.map<DemoTask>((item) => item.id === id
      ? { ...item, stage: 'downloading', error: null }
      : item),
  };
}
...
function restoreTask(value: unknown): DemoTask | null {
  if (!isObject(value) || !exactKeys(value, ['id', 'workId', 'stage', 'progress', 'paused', 'error'])) return null;
  if (typeof value.workId !== 'string' || !readyWorkIds.has(value.workId)
    || value.id !== `demo-${value.workId}`
    || typeof value.stage !== 'string' || !taskStages.has(value.stage as TaskStage)
    || typeof value.progress !== 'number' || !Number.isInteger(value.progress)
    || value.progress < 0 || value.progress > 100 || typeof value.paused !== 'boolean') return null;
  ...
  if (stage === 'error' ? value.error !== demoError : value.error !== null) return null;
  if ((stage === 'completed' || stage === 'error') && value.paused) return null;
  return {
    id: `demo-${value.workId}`, workId: value.workId, stage, progress: value.progress,
    paused: value.paused, error: stage === 'error' ? demoError : null,
  };
}
...
export function restoreDemoState(raw: string | null): DemoState {
  ...
  let value: unknown;
  try { value = JSON.parse(raw); } catch { return initialDemoState(); }
  if (!isObject(value) || !exactKeys(value, ['version', 'tasks', 'online', 'paused', 'closed'])
    || value.version !== 1 || !Array.isArray(value.tasks) || value.tasks.length > readyWorkIds.size
    || typeof value.online !== 'boolean' || typeof value.paused !== 'boolean'
    || typeof value.closed !== 'boolean') return initialDemoState();
  ...
  return { version: 1, tasks, online: value.online, paused: value.paused, closed: value.closed };
}
