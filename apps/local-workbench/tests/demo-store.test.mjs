...
import {
  initialDemoState, enqueueWorks, tickDemo, toggleTaskPause, retryDemoTask,
  setDemoPaused, setDemoOnline, closeDemo, reopenDemo, restoreDemoState,
...
const progressUntil = (state, predicate, limit = 100) => {
  for (let step = 0; step < limit && !predicate(state); step += 1) state = tickDemo(state);
...
test('initial tasks show download, imported waiting for sync, and a retryable interruption', () => {
  ...
  assert.deepEqual(state.tasks.map(({ workId, stage, progress }) => ({ workId, stage, progress })), [
  ...
  assert.equal(findTask(next, 'moon').stage, 'completed', 'sync acknowledgement is independent of the local download');
  state.tasks[0].progress = 1;
  assert.equal(initialDemoState().tasks[0].progress, 35, 'defaults must not share mutable tasks');
});

test('enqueue accepts known ready work once and refuses owned, review and forged items', () => {
  const state = initialDemoState();
  const result = enqueueWorks(state, [
    work('rain'), work('rain'), work('sea'), work('summer'), work('echo'),
    { ...work('summer'), status: 'ready' }, { ...work('echo'), status: 'ready' },
    { ...work('rain'), id: 'unknown-work' }, { ...work('flight'), status: 'review' },
  ]);
  assert.deepEqual(result.tasks.map((task) => task.workId), ['sea', 'moon', 'train', 'rain']);
  assert.deepEqual(state, initialDemoState(), 'enqueue must not mutate its input');
  assert.deepEqual(enqueueWorks(result, [work('rain')]), result);
  assert.deepEqual(enqueueWorks(closeDemo(result), [work('bookshop')]), closeDemo(result));
});
...
test('each local stage is observable and confirmed completion cannot enqueue twice', () => {
  ...
  for (const stage of ['queued', 'downloading', 'verifying', 'packing', 'importing', 'sync_pending', 'completed']) {
    assert.ok(seen.has(stage), `${stage} must be visible before the next stage`);
  }
...
test('offline imports wait for sync while other local work continues without re-downloading', () => {
  let state = setDemoOnline(enqueueWorks(emptyState(), [work('rain'), work('flight')]), false);
  state = progressUntil(state, (current) => current.tasks.every((task) => task.stage === 'sync_pending'));
  ...
  state = progressUntil(state, (current) => {
    assert.ok(current.tasks.every((task) => ['sync_pending', 'completed'].includes(task.stage)));
...
test('one local task advances per tick; task pause and resume preserve explicit choices', () => {
  ...
  state = toggleTaskPause(state, 'demo-sea');
  assert.equal(findTask(state, 'rain').paused, false, 'resuming sea must not auto-pause rain');
  const prior = structuredClone(state);
  state = tickDemo(state);
  assert.equal(findTask(state, 'sea').progress, 43);
  assert.deepEqual(findTask(state, 'rain'), findTask(prior, 'rain'), 'later progress waits for its turn');
  assert.deepEqual(tickDemo(setDemoPaused(state, true)), setDemoPaused(state, true));
});
...
test('closing stops execution and additions; restoring and reopening keeps the confirmed queue', () => {
  ...
  assert.equal(reopenDemo(manuallyPaused).paused, true, 'reopening does not override a global pause');
});

test('a failed task does not block the queue and only errors can retry from retained progress', () => {
  let state = { ...initialDemoState(), tasks: [findTask(initialDemoState(), 'train')] };
...
test('persistence rejects corruption, future formats, unknown ids and impossible task states', () => {
  ...
  const changes = [
    ...
    (state) => { state.credentials = 'not allowed'; },
    (state) => { state.tasks.push({ ...state.tasks[0] }); },
    (state) => { state.tasks[0].id = 'real-executor-task'; },
    (state) => { state.tasks[0].workId = 'not-in-catalog'; },
    (state) => { state.tasks[0].workId = 'summer'; state.tasks[0].id = 'demo-summer'; },
    (state) => { state.tasks[0].workId = 'echo'; state.tasks[0].id = 'demo-echo'; },
    (state) => { state.tasks[0].stage = 'download-authorized'; },
    (state) => { state.tasks[0].progress = -1; },
    (state) => { state.tasks[0].progress = 101; },
    (state) => { state.tasks[0].progress = 35.5; },
    (state) => { state.tasks[0].stage = 'queued'; },
    (state) => { state.tasks[0].stage = 'completed'; },
    (state) => { state.tasks[0].error = 'unexpected message'; },
    (state) => { state.tasks[1].progress = 99; },
    (state) => { state.tasks[2].error = null; },
    (state) => { state.tasks[2].error = '<script>not trusted</script>'; },
    (state) => { state.tasks[2].paused = true; },
    (state) => { state.tasks[0].destination = '/not-a-real-library'; },
  ];
  ...
  for (const raw of invalid) assert.deepEqual(restoreDemoState(raw), initialDemoState(), String(raw));
  assert.deepEqual(restoreDemoState(JSON.stringify(emptyState())), emptyState(), 'an empty valid queue is preserved');
});
...
test('commands and serialization leave frozen inputs untouched and all reached states restore', () => {
  ...
  const transitions = [
    enqueueWorks(state, [work('rain')]), tickDemo(state), toggleTaskPause(state, 'demo-sea'),
    retryDemoTask(state, 'demo-train'), setDemoPaused(state, true), setDemoOnline(state, false),
    closeDemo(state), reopenDemo(state),
  ];
  for (const result of transitions) assert.deepEqual(restoreDemoState(JSON.stringify(result)), result);
