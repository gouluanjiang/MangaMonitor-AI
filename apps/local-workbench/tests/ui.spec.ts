...
test("library search survives a visit to the independent detail page", async ({
  page,
}, testInfo) => {
  await expect(page.locator('[data-testid^="card-"]')).toHaveCount(8);
  await page.screenshot({
    path: testInfo.outputPath("preview-library.png"),
    fullPage: true,
    animations: "disabled",
  });
  ...
  await page.screenshot({
    path: testInfo.outputPath("preview-detail.png"),
    fullPage: true,
    animations: "disabled",
  });
...
test("the author link from a queue detail opens that author in the library", async ({
  page,
}) => {
  await page.getByTestId("nav-queue").click();
  await page
...
test("one batch confirmation queues two works and prevents duplicate selection", async ({
  ...
  await page.screenshot({
    path: testInfo.outputPath("preview-queue.png"),
    fullPage: true,
    animations: "disabled",
  });
