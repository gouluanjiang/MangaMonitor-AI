...
fn usage() -> ! {
    eprintln!(
        "usage: assistant-executor-queue --state <dir> [--gates <assistant-task-gates.json>] [--offset N --limit N]"
    );
    std::process::exit(2);
}
...
fn main() {
    ...
    let result = (|| -> Result<Value, String> {
        ...
        executor_handoff::queue_view(&state, &ledger, offset, limit)
    })();
    match result {
        Ok(value) => println!("{}", serde_json::to_string_pretty(&value).expect("queue serializes")),
        Err(error) => {
            eprintln!("{error}");
            std::process::exit(1);
        }
