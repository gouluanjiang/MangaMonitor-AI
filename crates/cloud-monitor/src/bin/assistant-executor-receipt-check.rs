use cloud_monitor::{assistant_task_gate, executor_handoff, persistence};
use serde_json::Value;
use std::{fs, path::{Path, PathBuf}};

fn usage() -> ! {
    eprintln!(
        "usage: assistant-executor-receipt-check --state <dir> --receipt <receipt.json> [--gates <assistant-task-gates.json>]"
    );
    std::process::exit(2);
}
...
fn main() {
    ...
    let result = (|| -> Result<Value, String> {
        ...
        let receipt: executor_handoff::ExecutorReceipt = serde_json::from_slice(
            &fs::read(receipt_path).map_err(|_| "EXECUTOR_RECEIPT_READ")?,
        )
        .map_err(|_| "EXECUTOR_RECEIPT_JSON")?;
        executor_handoff::receipt_view(&state, &ledger, &receipt)
    })();
    match result {
        Ok(value) => println!("{}", serde_json::to_string_pretty(&value).expect("receipt view serializes")),
        Err(error) => {
            eprintln!("{error}");
            std::process::exit(1);
        }
