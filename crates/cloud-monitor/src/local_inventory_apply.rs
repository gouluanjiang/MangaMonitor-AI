...
fn atomic_replace_file(target: &Path, replacement: &Path) -> Result<(), String> {
    ...
    let replacement_wide: Vec<u16> = replacement
        ...
        .chain(Some(0))
        .collect();
    // REPLACEFILE_WRITE_THROUGH is documented as unsupported. The replacement
    // file is explicitly synced before this call and the target is synced again
    // after it. We intentionally do not ignore ACL/merge errors.
    let ok = unsafe {
        ReplaceFileW(
            target_wide.as_ptr(),
...
/// Apply one exact V1.6 authorization to the local authoritative inventory.
///
/// Retry is safe: if the exact post-state is already present, the function
/// reconstructs the unique pre-state, re-runs V1.6 against that pre-state and
/// returns `ALREADY_APPLIED_EXACT_POST` without writing again.
pub fn apply(
    state_dir: &Path,
    report: &LocalInventoryRescanReport,
    candidate: &InventoryUpdateCandidate,
    ...
    if current_hash == authorization.inventory_snapshot_hash {
        ...
        // Narrow the local race window: once the fully synced replacement is
        // ready, reload all current state and re-run V1.6 immediately before
        // replacing the one authorized file.
