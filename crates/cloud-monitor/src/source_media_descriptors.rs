...
impl MediaDescriptor {
    /// Source format still binds the exact remote URL. The explicit JPEG
    /// transform changes only the decoded output and its canonical suffix.
    pub fn stored_format(&self) -> &str {
        if self.source_format == "webp" && self.transform == "JM_SCRAMBLE_BLOCKS_JPEG" {
            "jpg"
        } else {
            &self.source_format
        }
...
/// Selected only by the desktop task's persisted, approval-bound output policy.
/// Legacy descriptors and checkpoint hashes are unchanged unless selected.
pub fn jm_jpeg_output(descriptors: &mut SourceMediaDescriptorSet) -> Result<(), String> {
    if descriptors.source != "jm" {
        return Err("INVALID_JM_MEDIA_TRANSFORM".into());
    }
    for chapter in &mut descriptors.chapters {
        for media in &mut chapter.media {
            if media.source_format == "webp" && media.transform == "JM_SCRAMBLE_BLOCKS" {
                media.transform = "JM_SCRAMBLE_BLOCKS_JPEG".into();
                media.relative_path = expected_path(
                    chapter.chapter_order,
                    &chapter.chapter_id,
                    media.image_index,
                    "jpg",
                );
            } else if media.source_format != "gif" || media.transform != "NONE" {
                return Err("INVALID_JM_MEDIA_TRANSFORM".into());
            }
        }
    }
    ...
    match media.source_format.as_str() {
        ...
        "webp"
            if matches!(
                ...
                "JM_SCRAMBLE_BLOCKS" | "JM_SCRAMBLE_BLOCKS_JPEG"
            ) =>
        {
