...
fn layout(
    ...
    let mut comic_metadata = match record.source {
        Source::Jm => {
    ...
    // Do not add a null field to older tasks: their generated bytes are bound
    // into resume/output manifests. This explicit field is the version snapshot
    // and does not change upstream compatibility placeholders.
    if let Some(version_updated_at) = &metadata.version_updated_at {
        comic_metadata["mangaMonitor"]["versionUpdatedAt"] = json!(version_updated_at);
    }
